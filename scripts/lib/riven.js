...
function Ø(s, network = RIVEN.network) {
  ...
  if (id.indexOf(' ') > -1) {
    ...
    const port_id = split[1];
    return network[node_id] && network[node_id].ports[port_id] ?
      network[node_id].ports[port_id] : null;
  } else if (network[id]) {
    return network[id];
  } else {
    return new Node(id);
...
function Node(id, rect = {x: 0, y: 0, w: 2, h: 2}) {
  ...
  this.create = (pos = {x: 0, y: 0}, type = Node, ...params) => {
  ...
  this.mesh = (pos, n) => {
    ...
    if (n instanceof Array) {
      for (let id in n) {
        n[id].parent = node;
        node.children[node.children.length] = n[id];
        node.update();
      }
    } else {
      n.parent = node;
  ...
  this.connect = (q, type = ROUTE_TYPES.output) => {
    if (q instanceof Array) {
      ...
      this.ports[type === ROUTE_TYPES.request ?
        'request' : 'output'].connect(`${q} ${type === ROUTE_TYPES.request ?
          'answer' : 'input'}`, type);
    }
  }
